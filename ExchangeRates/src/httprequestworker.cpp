...
{
    QRandomGenerator(QDateTime::currentDateTime().currentMSecsSinceEpoch());
...
void HttpRequestWorker::execute(HttpRequestInput *input, bool auth) {
    ...
    if (input->var_layout == ADDRESS || input->var_layout == URL_ENCODED) {
        ...
        boundary = "__-----------------------"
            + QString::number(QDateTime::currentDateTime().currentMSecsSinceEpoch())
            + QString::number(QRandomGenerator().generate());
        ...
        for (QList<HttpRequestInputFileElement>::iterator file_info = input->files.begin(); file_info != input->files.end(); file_info++) {
            ...
            if (
                file_info->local_filename.isNull() || file_info->local_filename.isEmpty()
                || file_info->variable_name.isNull() || file_info->variable_name.isEmpty()
            ...
            if (file_info->request_filename.isNull() || file_info->request_filename.isEmpty()) {
    ...
    // TODO: Now it's hard coded here, find a better solution
    if(auth)
    {
        request.setRawHeader("Content-Type", "application/json");
        request.setRawHeader("Authorization", "Basic ZTlmODlhN2VmMzk2MjkyMmYyMzM0MDJlNmI4NmU2NDk6OGI0NjU0MjNlOWFiZDE3ZTc2ZTcxMWUzNjk0MTZkZGM=");
    }
